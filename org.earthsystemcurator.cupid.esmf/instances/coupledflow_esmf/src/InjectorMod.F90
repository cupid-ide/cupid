! $Id: InjectorMod.F90,v 1.14 2011/06/30 05:58:24 theurich Exp $
!
!-------------------------------------------------------------------------
!BOP
...
!EOE
   ...
   20 continue
     ...
     print *, "INJECTOR START TIME: "
     call ESMF_TimePrint(datablock%inject_start_time)
     print *, "INJECTOR STOP TIME: "
     call ESMF_TimePrint(datablock%inject_stop_time)
...
!EOPI
      ! Local variables
        ...
        do i=1, datacount
           ...
           if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
           if(count .lt. 1) then
                !print *, "INJECTOR: copying importState to exportState: ", trim(datanames(i))
             call ESMF_StateGet(importState, datanames(i), thisfield, rc=rc)
             if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
             call ESMF_StateAdd(exportState, (/thisfield/), rc=rc)
             if(rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
