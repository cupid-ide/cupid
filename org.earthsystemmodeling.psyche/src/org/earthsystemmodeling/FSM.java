...
import org.earthsystemmodeling.generator.PsycheGenerator;
...
public class FSM<RootType extends EObject> {
	...
	protected PsycheGenerator generator;
	...
	public FSM(Language language, IProject project, Class<?> queryClass, Class<?> transformations, ILog log) {
		...
		this.generator = new PsycheGenerator();  //just used for "toClassName" methods
	...
	*/
	...
	private static List<PathExprTerm> linearizePathExpr(PathExpr pathExpr, List<PathExprTerm> segments) {
		...
		if (pathExpr instanceof PathExprTerm) {
			PathExprTerm pet = PsycheFactory.eINSTANCE.createPathExprTerm();
		...
		if (mappingElement == null) {
			...
			try {
				...
				if (replaceVal != null) {
					...
					Expr replaceValObj = PsycheFactory.eINSTANCE.createExpr();
					replaceValObj.setExpr(PsycheFactory.eINSTANCE.createLocalExpression());
