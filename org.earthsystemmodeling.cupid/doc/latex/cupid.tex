...
Instructions for downloading and installing Cupid (and the Eclipse IDE itself) are located on the Cupid web site at \href{https://www.earthsystemcog.org/projects/cupid/installation}{https://www.earthsystemcog.org/projects/cupid/installation}.
...
\begin{enumerate}
\item Open the NUOPC View

From the Eclipse menu, choose Window $\rightarrow$ Show View $\rightarrow$ Other... and then select \emph{NUOPC View} from the \emph{NUOPC} folder.
...
\item In the NUOPC View, find the element \emph{NUOPC\_CompDerive} under the \emph{SetServices} element. The yellow arrow icon indicates that the element maps to a subroutine call. Double-click the element and the corresponding call will be highlighted in the source code. The call to \texttt{NUOPC\_CompDerive} indicates that this component is deriving from one of the generic NUOPC components, in this case the \texttt{NUOPC\_Driver} component. 
...
\item Now open the file \empt{atm.F90} by double-clicking in the Project Explorer.  The NUOPC View should now update with content from that file.
...
The NUOPC View shows that atm.F90 is code for a NUOPC Model.
