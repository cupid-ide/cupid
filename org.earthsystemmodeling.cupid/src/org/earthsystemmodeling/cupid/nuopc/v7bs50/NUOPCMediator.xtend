	...
	override reverse() {
		...
		var rs = '''module(_moduleID, _compUnitID, _medName), 
		            compilationUnit(_compUnitID, _filename, _path),
   					uses(_uid, _mid, 'NUOPC_Mediator').'''.execQuery
		try {
			if (rs.next) {
...
subroutine «subroutineName»(«paramGridComp», «paramRC»)
    ...
    call NUOPC_ClockPrintCurrTime(clock, &
        "------>Advancing «_parent._parent._parent.mediatorName» from: ", rc=«paramRC»)
    if (ESMF_LogFoundError(rcToCheck=«paramRC», msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out
...
subroutine «subroutineName»(«paramGridComp», «paramRC»)
    ...
    call NUOPC_StateSetTimestamp(exportState, clock, rc=«paramRC»)
    if (ESMF_LogFoundError(rcToCheck=«paramRC», msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
